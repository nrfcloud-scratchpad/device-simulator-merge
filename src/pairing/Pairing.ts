...
const PairingStateType = t.refinement(
  t.String,
  (s: string) =>
    s && (Object.keys(State.STATE).map(k => State.STATE[k]) as any).includes(s),
  'State',
);
...
const NonEmptyStringType = t.refinement(
  t.String,
  (s: any) => s && s.length > 0,
  'NonEmptyStringType',
);
...
export class StatePaired extends State {
  ...
  constructor(topics?: PairingTopics) {
    ...
    this.topics = topics
      ? new PairingTopics(topics.c2d, topics.d2c)
      : undefined;
  }
  ...
  update(previous: State): State {
    if (
      previous &&
      previous.state === State.STATE.paired &&
      previous instanceof StatePaired
    ) {
      return new StatePaired(
        previous.topics
          ? new PairingTopics(
              this.topics && this.topics.c2d
                ? this.topics.c2d
                : previous.topics.c2d,
              this.topics && this.topics.d2c
                ? this.topics.d2c
                : previous.topics.d2c,
            )
          : undefined,
  ...
  static copy(statePaired: StatePaired): StatePaired {
    return new StatePaired(
      statePaired.topics
        ? new PairingTopics(statePaired.topics.c2d, statePaired.topics.d2c)
        : undefined,
    );
  }
}
...
export class StatePatternWait extends State {
  ...
  update(previous: State): State {
    if (
      previous &&
      previous.state === State.STATE.patternWait &&
      previous instanceof StatePatternWait
    ) {
  ...
  static copy(statePatternWait: StatePatternWait) {
    return new StatePatternWait(
      new PairingConfig(
        statePatternWait.config.method,
        statePatternWait.config.length,
        statePatternWait.config.iteration,
      ),
    );
  }
}
