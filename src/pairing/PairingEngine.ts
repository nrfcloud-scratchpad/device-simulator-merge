...
export interface IPairingEngine {
  ...
  on(
    event: 'pairingUpdate',
    listener: (state: State, status: PairingStatus) => void,
  ): this;
  ...
  on(
    ...
    listener: (pairingStatus: PairingStatus) => void,
  ): this;
}
...
export class PairingEngine extends EventEmitter implements IPairingEngine {
  ...
  private async cancelRetrievePattern(): Promise<void> {
    if (this.pairingMethods && this.selectedPairingMethod) {
      const foundMethod = this.pairingMethods.find(
        method => method.methodName === this.selectedPairingMethod!.methodName,
      );
  ...
  private stateWaitingForPattern(state: StatePatternWait) {
    if (this.pairingMethods) {
      ...
      if (!foundMethod) {
        ...
        foundMethod
          .retrievePattern(state.config.length)
          .then(pattern => {
      ...
      throw new Error(
        'No pairing methods registered but pairing is requested.',
      );
    }
  }
    ...
    switch (pairing.state) {
      ...
      case State.STATE.patternWait:
        ...
        return new StatePatternWait(
          new PairingConfig(config.method, config.length, config.iteration),
        );
      ...
      case State.STATE.paired:
        ...
        return new StatePaired(
          topics ? new PairingTopics(topics.c2d, topics.d2c) : undefined,
        );
      default:
  ...
  private emitShadowUpdate(pairing: Pairing) {
    ...
    if (
      this.previousPairing &&
      pairing.state &&
      this.previousPairing.state !== pairing.state
    ) {
  ...
  async updatePairingState(pairing: Pairing) {
    ...
    switch (localState.state) {
      ...
      case 'pattern_mismatch':
        await this.cancelRetrievePattern();
        break;
    }
