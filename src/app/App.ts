...
export class App {
  private readonly pairingEngine: IPairingEngine;
  private readonly hostConnection: IHostConnection;
  ...
  constructor(
    ...
    this.services = Array.from(sensors.entries()).map(([name, sensor]) =>
      createService(name, sensor, this.sendMessage),
    );
  }

  private readonly sendMessage: SendMessage = (timestamp, message) => {
    const timeStamp = new Date(timestamp).toISOString();
    console.debug(
      `Timestamp in message #${this
        .messagesSent++}, ${timeStamp} removed from message, since firmware implementation does not support it yet.`,
    );
    console.debug(
      `messageId not sent in message since firmware implementation does not have it.`,
    );

    this.hostConnection.sendMessage(JSON.stringify(message)).catch(error => {
      console.error(
        `Error sending sensor data to nRF Cloud. Error is ${error.message}.`,
      );
    });
  };
  ...
  private async startApplication(pairing: Pairing): Promise<void> {
    if (pairing.state === 'paired') {
      if (pairing.topics && pairing.topics.d2c) {
        await this.hostConnection.setTopics(
          pairing.topics.c2d,
          pairing.topics.d2c,
        );
        ...
        console.warn(
          'Paired but application topics are NOT provided by nRF Cloud.',
        );
      }
    }
  }
  ...
  private setupPairing() {
    this.pairingEngine.on('pairingUpdate', async (state, status) => {
      console.debug(
        `updating shadow -> reported.pairing: ${JSON.stringify(
          state,
        )} status: ${JSON.stringify(status)}`,
      );
      await this.hostConnection.updateShadow({
    ...
    this.hostConnection.on('shadowDelta', async (delta: ShadowModelDesired) => {
      if (delta.pairing) {
        ...
        console.debug(
          `shadow; json data not related to pairing: ${JSON.stringify(delta)}`,
        );
  ...
  async main() {
    ...
    this.hostConnection.on('message', (message: any) => {
      ...
      console.info(
        `Received message (ignoring it) ${JSON.stringify(demopackMessage)}`,
      );
    });
