...
export class AWSIoTHostConnection extends EventEmitter
  ...
  async updateShadow(reported: ShadowModelReported): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      this.mqtt!.publish(
        `${this.getShadowBaseTopic()}/update`,
        JSON.stringify(root),
        undefined,
        error => {
          if (error) {
  ...
  connect(): Promise<void> {
    return new Promise<void>((resolveConnect, rejectConnect) => {
      const connectOptions: DeviceOptions = {
        ...
        host:
          this.config.brokerHostname ||
          'a2n7tk1kp18wix.iot.us-east-1.amazonaws.com',
        debug: false,
      };
      ...
      this.mqtt!.on('message', (topic: string, payload: any) => {
        ...
        switch (topic) {
          ...
          case `${shadowBaseTopic}/update/delta`:
            ...
            const delta: any = <ShadowModelDesired>(
              Object.assign({}, parsed.state)
            );
